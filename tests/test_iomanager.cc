...
void do_io_write1(){
    ...
    SYLAR_LOG_INFO(g_logger) << "do_io_write1";
    // int readlen = 0;
    readlen = write(sockfd, buf, readlen);
    if(readlen > 0) {
        buf[readlen] = '\0';
        SYLAR_LOG_INFO(g_logger) << "write " << readlen << " bytes, read: " << buf;
    } else if(readlen == 0) {
    ...
    sylar::IOManager::GetThis()->cancelEvent(sockfd, sylar::IOManager::WRITE);
...
void do_io_read() {
   ...
   sylar::IOManager::GetThis()->addEvent(sockfd, sylar::IOManager::WRITE, do_io_write1);
}
